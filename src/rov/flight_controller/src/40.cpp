...
FlightController::FlightController() : Node("flight_controller") {
    ...
    // pos P controller (maybe we want PI, TODO: fix I controller)
    position_pid = PID<Eigen::Vector<float, 6>>(true, false, false);
    // vel PD controller
    ...
    for (uint64_t i; i < NUM_THRUSTERS; i++) {
        ...
        const double* pos = this->get_parameter(parameter_name_base + "Position").as_double_array().data();
        const double* fwd = this->get_parameter(parameter_name_base + "Forward").as_double_array().data();
        Thruster thruster = {
